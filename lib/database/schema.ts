...
export async function migrateDatabase(db: SQLiteDatabase): Promise<void> {
  ...
  // All schema steps run in one transaction. SQLite DDL is transactional, and
  // user_version is part of the database header, so if the app is killed
  // mid-migration (e.g. during the first launch after an update) everything —
  // including the version bump — rolls back atomically. Without this, a partial
  // migration would re-run an already-applied ADD COLUMN on the next launch and
  // crash-loop with "duplicate column name". WAL mode is set outside because
  ...
  await db.withTransactionAsync(async () => {
    ...
    if (currentVersion < 20) {
      await db.execAsync(`
        CREATE TABLE IF NOT EXISTS check_in_drafts (
          id       INTEGER PRIMARY KEY CHECK (id = 1),
